...
if __name__ == "__main__":
	...
	for filename in os.listdir(folder):
		...
		if img is None:
			print(f'Unable to locate {imagePath}')
			os.sys.exit()

		# detect faces in the frame and determine whether or
		# not they are wearing a face mask
		locations = detectFaces(img, faceNet)
		if len(locations) != 0:
			(locations, predictions) = detectMasks(img, locations, maskNet)
			...
			for (box, prediction) in zip(locations, predictions):
				...
				cv2.putText(img, label, (startX, startY - 10),
					cv2.FONT_HERSHEY_SIMPLEX, 1.5, colour, 2)
				cv2.rectangle(img, (startX, startY), (endX, endY), colour, 4)
			
			end = datetime.now()	
			print(end - start)
			cv2.imwrite(f'{folder}/output_{filename}', img)